...
(async () => {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const speed = (mapNormalizedToRange($mouseX, 1, 4));
        const shift = (mapNormalizedToRange($mouseY, 0, 1));
        const cut = (mapNormalizedToRange($mouseY, 400, 1200));

        faustNode.setParamValue("/sound/Multiplier", speed);
        faustNode.setParamValue("/sound/Shift", shift);
        faustNode.setParamValue("/sound/Cut", cut);
    }
    );
...
function creatRandomNotes(faustNode) {
    noteNumbers = [60, 62, 64, 66, 68, 70, 72, 74]

    noteNumbers.map(value => {
        const note  = Math.floor(Math.random() * 60) + 20;
        faustNode.setParamValue(`/sound/Note_${value}`, note);
