...
(async () => {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const freq = (mapNormalizedToRange($mouseX, 200, 2400)).toFixed(2);
        const mod = (mapNormalizedToRange($mouseY, 4, 80)).toFixed(2);
        const mx = (mapNormalizedToRange($mouseX, 1, 10)).toFixed(2);
        const my = (mapNormalizedToRange($mouseX, 1, 20)).toFixed(2);

        faustNode.setParamValue("/sound/Osc_400/Freq", freq);
        faustNode.setParamValue("/sound/Osc_800/Freq", freq * mx);
        faustNode.setParamValue("/sound/Osc_1200/Freq", freq * mx * my);
        faustNode.setParamValue("/sound/Osc_1600/Freq", freq * mx + my);

        faustNode.setParamValue("/sound/Osc_400/Mod", mod);
        faustNode.setParamValue("/sound/Osc_800/Mod", mod * my);
        faustNode.setParamValue("/sound/Osc_1200/Mod", mod * my * 0.1);
        faustNode.setParamValue("/sound/Osc_1600/Mod", mod * my * 0.2);
    });
