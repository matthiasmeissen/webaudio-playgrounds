...
(async () => {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const speed2 = (mapNormalizedToRange($mouseY, 0.25, 4));

        faustNode.setParamValue("/sound/Voice_1/Multiplier_1", speed1);
        faustNode.setParamValue("/sound/Voice_2/Multiplier_2", speed2);
    }
    );

    document.addEventListener("click", (e) => {
        creatRandomNotes(faustNode, 1);
        creatRandomNotes(faustNode, 2);
    });
...
function creatRandomNotes(faustNode, n) {
    for(let i = 0; i < 4; i++) {
        const note  = Math.floor(Math.random() * 60) + 20;
        faustNode.setParamValue(`/sound/Voice_${n}/Note_${i}`, note);
