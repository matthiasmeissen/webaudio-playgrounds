...
document.addEventListener("mousemove", (e) => {
    ...
    const freq = (mapNormalizedToRange(mouseX, 20, 2000));
    const amp = (mapNormalizedToRange(mouseY, 0.0, 1.0));

    faustNode.setParamValue("/sound/Freq", freq);
    faustNode.setParamValue("/sound/Amlitude_Modulation", amp);
});

document.addEventListener("keydown", (e) => {
    if (e.key == "1") {
        faustNode.setParamValue("/sound/Reset", 1);
    } else if (e.key == "2") {
        faustNode.setParamValue("/sound/Pause", 1);
    } else if (e.key == "3") {
        faustNode.setParamValue("/sound/Use_Custom", 1);
    }
})

document.addEventListener("keyup", (e) => {
    if (e.key == "1") {
        faustNode.setParamValue("/sound/Reset", 0);
    } else if (e.key == "2") {
        faustNode.setParamValue("/sound/Pause", 0);
    } else if (e.key == "3") {
        faustNode.setParamValue("/sound/Use_Custom", 0);
    }
})
